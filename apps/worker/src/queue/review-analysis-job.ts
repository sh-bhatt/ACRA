...
function parseQueueMessage(
  ...
  if (typeof value === "string") {
    try {
      normalizedValue = JSON.parse(value) as unknown;
    } catch {
      throw new Error(
...
export async function claimAndInspectReviewAnalysisJob(
  ...
  const parsedQueueRow =
    claimedQueueRowSchema.safeParse(rawQueueRow);
  ...
  if (review.status !== "queued") {
    throw new Error(
      [
        "Expected review status queued,",
        `received ${review.status}.`,
      ].join(" "),
    );
  }
  ...
  for (const file of files) {
    ...
    const downloadedSize = Buffer.byteLength(
      sourceText,
      "utf8",
    );
    ...
    if (downloadedSize !== file.size_bytes) {
      throw new Error(
        [
    ...
    if (downloadedHash !== file.code_hash) {
      throw new Error(
  ...
  console.log(
    "[queue] claim-and-inspect completed successfully",
  );

  console.log(
    "[queue] message intentionally left unarchived",
  );
}