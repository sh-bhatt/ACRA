...
export async function claimAndRunStaticAnalysisJob(
    ...
    let message: ReviewAnalysisMessage;
    ...
    try {
        ...
        await deleteInvalidReviewAnalysisJob(
            supabase,
            claimedJob.msg_id,
        );
    ...
    try {
        ...
        for (const file of files) {
            ...
            console.log(
                `[eslint] rules=${detectedRuleIds.length > 0
            ...
            await recordReviewAnalysisFailure(
                supabase,
                {
                    ...
                    safeErrorMessage:
                        failure.safeMessage,
                    isUnrecoverable:
                        failure.isUnrecoverable,
                },
            );

        console.error(
            [
                "[queue] static-analysis attempt failed",
        ...
        if (
            failureResult.action === "retry"
        ) {
            console.log(
                [
                    "[queue] message left unarchived",
                    "for retry after visibility timeout",
                ].join(" "),
            );
        ...
        console.error(
            `[review] marked failed review_id=${message.reviewId}`,
        );
